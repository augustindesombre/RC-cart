...
#define SPEED_MAX_TEST      900         // [-] Maximum speed for testing
...
{ 
  ...
  rc_steer = receiver.getMap(1);
  rc_speed = receiver.getMap(2);

  if (abs(rc_steer - 50) < 5) {
    rc_steer = 50;
  }

  if (abs(rc_speed - 50) < 5) {
    rc_speed = 50;
  }
  

  Send(scaling*(rc_steer*10-500), scaling*(rc_speed*10-500));
